...
#include "Chip8/Actions.hpp"
#include <type_traits>

using OpcodeType=short;
using RegisterIndex=short;
namespace Assemble  {
...
protected:
  ...
  Chip8InstEmu():emulator(std::make_unique<C8::Registers>(), 
...
TEST_F(Chip8InstEmu, Jump) {
  C8::MemoryAddress jumpedAddr = 0x123;
  InjectInst(
    C8::Registers::CHIP8_INITIAL_ADDRESS,
    Assemble::JumpTo(jumpedAddr)
  );
  Step();
  EXPECT_EQ(emulator.GetRegistersPtr()->GetProgramCounter(), jumpedAddr);
}
TEST_F(Chip8InstEmu, RetFromSubroutine) {
  C8::MemoryAddress jumpedAddr = 0x123;
  InjectInst(
    C8::Registers::CHIP8_INITIAL_ADDRESS,
    Assemble::CallSub(jumpedAddr)
  );
  InjectInst(
    jumpedAddr,
    Assemble::Return()
  ...
  Step(2);
  EXPECT_EQ(emulator.GetRegistersPtr()->GetProgramCounter(), C8::Registers::CHIP8_INITIAL_ADDRESS);
}
