...
namespace C8::Opcode {
  ...
  constexpr inline auto DecodeArgs(unsigned short data, ArgsType type) -> Args {
    Args ret {};
    constexpr auto ToReg = [](unsigned short v) -> C8::Register {
      return static_cast<C8::Register>(v);
    };
    switch (type) {
      case ArgsType::NNN:
        ret.NNN = data & 0x0FFF;
...
namespace C8::Actions {
  ...
  public:
    ...
    ~BaseChip8InstructionAction() override = default;
  };
  ...
  public:
    ...
    void Execute() override { m_Emulator.ClearScreen(); }
  };
  ...
  public:
    ...
    void Execute() override {
      m_Emulator.GetMemoryPtr()->PushStack(
          m_Emulator.GetRegistersPtr()->GetProgramCounter() + OPCODE_SIZE);
      m_Emulator.GetRegistersPtr()->SetProgramCounter(m_Addr);
    }
  };
  ...
  public:
    SkipNotEqualImm(C8::Register x,
                    C8::MemoryValue value,
