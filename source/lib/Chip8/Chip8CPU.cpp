...
C8::CPU::CPU(Emulator& emulator)
    : m_Emulator(emulator) {}

auto C8::CPU::Next() -> BaseInstructionActionPtr {
  ...
  const auto change_type = [&type, &args, opcode](auto func,
                                                  Opcode::ArgsType a) {
...
#define XNN_CALL(action) \
  ret = std::make_unique<action>(args.XKK.x, args.XKK.kk, m_Emulator);
#define XYN_CALL(action) \
  ret = std::make_unique<action>( \
      args.XYN.x, args.XYN.y, args.XYN.n, m_Emulator);
#define XY_CALL(action) \
  ret = std::make_unique<action>(args.XY.x, args.XY.y, m_Emulator);
